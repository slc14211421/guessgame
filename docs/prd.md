...
| 优先级 | 功能 | 说明 |
| --- | --- | --- |
| P0 | 首页分组列表 | 以横向滑动卡片展示所有词语分组和词语数量。 |
...
| 分类管理入口 | 进入分类管理页。 |
| 词语分组列表 | 以“选择猜词卡片”为标题，使用与猜词时间一致的外框区域，横向滑动展示全部分类，每屏约展示 2.5 个卡片。 |
...
3. 用户点击分组后，该分组进入选中状态。
4. 未选择分组时，开始游戏按钮置灰或点击后提示“请选择词语分组”。
5. 已选择分组但分组无词语时，点击开始游戏提示“该分组暂无词语”。
6. 开始游戏前必须单选猜词时间，未选择时提示“请选择猜词时间”。
7. 点击可用的开始游戏按钮后，跳转倒计时页。
8. 首版首页不应用用户设置中的背景色和词条颜色。
...
4. 后续进入猜词模式时应用已保存设置。
5. 若背景色和词条颜色明显不可读，系统自动将词条颜色修正为当前背景下更易读的深色或浅色；蓝底白字等可读组合允许保留。
...
2. 猜词模式允许用户自定义背景色和词条颜色，但默认值和保存后的颜色组合需避免明显不可读。
...
2. 首页分类项需便于点击，横向卡片需清晰展示分类名称、词语数量和点击提醒。
...
| BR-15 | 用户保存明显不可读的颜色组合时，系统自动修正词条颜色；蓝底白字等可读组合允许保留。 |
...
2. 分组项在“选择猜词卡片”外框区域内以横向滑动卡片展示，卡片包含分类名称、词语数量和点击提醒。
3. 用户可选择某个词语分组。
4. 用户可从首页进入分类管理。
5. 首页内容区首行右侧可打开设置面板。
...
6. 重启应用后设置仍然生效。
7. 首页不应用背景色和词条颜色设置。
8. 明显不可读的颜色组合保存后会自动修正为可读组合，蓝底白字等可读组合允许保留。
