...
| 页面 | 路径 | 说明 |
| --- | --- | --- |
| 首页 | `/pages/index/index` | 横向滑动展示分类、开始游戏、设置入口。 |
...
7. 当 `STORAGE_VERSION` 升级且本地词库格式有效时，使用当前 `DEFAULT_WORDS` 同步系统默认词条，移除旧默认词条 id，并保留用户自行新增的非默认 id 词条。
...
6. `getSettings()` 和 `saveSettings()` 接收旧版或异常设置数据时统一归一化；若背景色与词条颜色明显不可读，使用 `utils/color.ts` 自动选择当前背景下更可读的深色或浅色词条颜色；蓝底白字等可读组合允许保留。
...
6. 词语分组使用带“选择猜词卡片”标题的外框区域承载横向 `scroll-view` 卡片，每屏约展示 2.5 个卡片；通过重复渲染三组分类并在滚动接近边界时回跳到中间副本，实现视觉上的环形循环滑动。
7. 首页 `onShow` 额外调用 `unlockPortrait()` 作为真机兜底，避免从倒计时或猜词页异常返回后方向残留。
...
| `handleCategoryCarouselScroll` | 处理首页分类卡片横向滚动，并在接近边界时回跳以保持环形循环。 |
...
1. 分类卡片需要展示分类名称、词语数量和点击提醒，分类选择区域标题为“选择猜词卡片”。
2. 分类卡片横向排列，每屏约展示 2.5 个卡片，并支持左右环形循环滑动。
3. 被选中的分类需要有明显选中态。
4. 猜词时间使用单选按钮展示，选项为 1、2、3、5、10 分钟。
5. 开始游戏按钮在无选择分类或无选择猜词时间时置灰。
6. 分类管理入口需要明确可见。
7. 首版首页不应用用户设置中的背景色和词条颜色，避免与猜词模式主题配置混淆。
8. 首页和分类管理页内容区不得重复展示与原生导航栏相同的页面标题。
9. 面板外框和标题优先复用全局 `.app-panel`、`.app-section-title`，页面级样式只保留横向滚动等局部差异。
...
3. 退出入口固定在右上角。
4. 词条、剩余时间和退出入口需要大字号或高对比；顶部控件应根据背景色自动选择深色或浅色。
5. 滑动区域应覆盖整个页面。
...
| `settings.ts` | 默认设置、保存设置、非法色值、缺失设置字段和明显不可读颜色兜底。 |
